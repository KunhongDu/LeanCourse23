...
attribute [instance] TopPair.isTotalTopologicalSpace

namespace TopPair

@[simp]
def sub (P : TopPair) : Type* := Subtype (P.sub')

end TopPair

def SubsetToTopPair (X : Type*) [TopologicalSpace X] (A : Set X) : TopPair where
  total := X
  sub' := A
  isTotalTopologicalSpace := by infer_instance

example (P : TopPair) : TopologicalSpace (Subtype P.sub') := by infer_instance

def toTopPair (X : Type*) [TopologicalSpace X] : TopPair where
  total := X
  sub' := fun _ ↦ false
  isTotalTopologicalSpace := by infer_instance

example (P : TopPair) : TopologicalSpace P.sub := by simp; infer_instance

example (α β : Type*) (f : α → β) (p : α → Prop) :  Subtype p → β := fun a ↦ f a.1


@[ext]
class PairMap (P₁ : TopPair) (P₂ : TopPair) extends ContinuousMap P₁.total P₂.total where
  image_sub : ∀ a : P₁.sub, P₂.sub' (toFun a.1)
...
/-
def PairMapToSubMap (f : PairMap P P') : C(β, β') where
  toFun := by
    intro b

    have : f.toFun (P.map b) ∈ Set.range P'.map := by
      apply f.image_sub (by simp)
...
/-
noncomputable def PairToSubMap {P P': TopPairCat} (f : P ⟶ P') : toPair' P.sub ⟶ toPair' P'.sub where
  toFun := by
    intro a
    have : f.toFun (P.isTopPair.map a) ∈ Set.range P'.isTopPair.map := by
      apply f.image_sub (by simp)
    exact Classical.choose this
  -- fun a: P.sub ↦ f.toFun (P.isTopPair.map a)
  continuous_toFun := by
    simp
    apply (Embedding.continuous_iff P'.isTopPair.isEmbedding).mpr
    set F := (P'.isTopPair.map ∘ fun a : P.sub ↦
    Classical.choose
      (_ : ∃ x, (fun x : P'.sub ↦ TopPair.map x = f.toFun (TopPair.map a)) x))
    have : F = f.toFun ∘ TopPair.map := by
      ext a
      simp
      sorry
    rw [this]
    apply Continuous.comp (by continuity) (Embedding.continuous P.isTopPair.isEmbedding)
  image_sub := by
    simp [PairRangeSub, Set.range]
-/
