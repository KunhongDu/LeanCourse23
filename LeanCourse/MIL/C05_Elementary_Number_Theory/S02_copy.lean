...
theorem add_assoc (m n k : MyNat) : add (add m n) k = add m (add n k) := by
  induction n
  case zero =>
    rw [add_comm m, zero_add, zero_add]
  case succ n hn =>
    rw [add_comm m]
    repeat rw [succ_add]
    rw [add_comm m, succ_add, add_comm n, add_comm (add n k), hn]
...
theorem mul_add (m n k : MyNat) : mul m (add n k) = add (mul m n) (mul m k) := by
  ...
  case succ n hn =>
    rw [succ_add]
    repeat rw [mul]
    rw [hn, add_assoc, add_comm (mul m k), ← add_assoc]
...
theorem succ_mul (m n : MyNat) : mul (succ m) n = add (mul m n) n := by
  ...
  case zero =>
    rw [mul, mul, zero_add]
  case succ n hn =>
    rw [mul, mul, hn, add_comm, succ_add, ← add_comm, add_assoc, add_comm n, add_comm (add (mul m n) m), succ_add, add_comm n, ← add_assoc]
